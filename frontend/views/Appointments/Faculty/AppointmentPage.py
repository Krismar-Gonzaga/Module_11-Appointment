...
class AppointmentPage_ui(QWidget):
    ...
    def _makeActionsCell(self, status, row_index=None):
        ...
        if status == "PENDING":
            ...
            try:
                # Open confirmation dialog on Approve
                approve_btn.clicked.connect(lambda _=False, r=row_index: self._openApproveDialog(r))
                # Open confirmation dialog on Deny
                deny_btn.clicked.connect(lambda _=False, r=row_index: self._openDenyDialog(r))
            except Exception:
                pass
            ...
            reschedule_button.clicked.connect(self.go_to_AppointmentReschedulePage.emit)
    ...
    def _openCancelDialog(self, row_index, status):
        ...
        if result == QtWidgets.QDialog.DialogCode.Accepted and row_index is not None:
            try:
                if status == "RESCHEDULED":
                    status_item = self._makeStatusItem("PENDING", "#F2994A")
                    self.tableWidget_8.setCellWidget(row_index, 5, self._makeActionsCell("PENDING", row_index))
                elif status == "APPROVED":
                    status_item = self._makeStatusItem("CANCELED", "#EB5757")
